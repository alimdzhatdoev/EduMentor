...
export default function StudentSchedule() {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <PageHeader title="Моё расписание" description="Запланированные занятия по неделям" />
              ...
              return (
                <div key={d} className="min-h-[120px]">
                  ...
                  <div className="space-y-1">
                    {dayLessons.map(l => (
                      <div key={l.id} className={cn('p-1.5 rounded-lg text-xs',
                        ...
                        <p className="truncate mt-0.5 hidden sm:block">{l.title}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {dayLessons.map(l => (
                  <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">{l.format === 'ONLINE' ? 'Онлайн' : 'Очно'}</p>
                    </div>
